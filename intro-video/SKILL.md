...
Ask what Video Agent acts on. You can tell it not to show a presenter, and it complies: a verified run omitted `avatar_id` entirely and rendered with no digital human in any frame. You can tell it to put a real environment behind the presenter (`presenter.scene: integrated`) the same way. You cannot tell it to stop narrating, to hold a duration, or to keep your pages — it writes and voices its own narration and paces its own timeline whatever the prompt says, and no field overrides that. What it acts on is prompted; the rest is Okou's to compose.
