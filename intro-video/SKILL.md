...
A completed provider job is already the user's file. Deliver its permanent URL immediately, with any known defect named in one sentence, then apply the lightweight technical check in [QA](references/qa.md). Ordinary native videos finish there. Content review is targeted to an explicit review request, a binding wording or preservation requirement, or a concrete observed problem; language choice and brand names alone do not trigger transcription. Caption editing, retiming, interpolation, and re-export belong to requested editing work, not routine QA. A paid retry still waits for the user's direction and carries a changed prompt.
