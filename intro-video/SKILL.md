...
The test is whether HeyGen has a mechanism for the requirement at all, not whether it guarantees the result — and an instruction the agent acts on is as much a mechanism as an API field. `No avatar` passes on the prompt: a verified run omitted `avatar_id` entirely and rendered with no digital human in any frame. A real environment behind the presenter (`presenter.scene: integrated`) passes the same way. Narration removal, source-audio retention, page/frame retention, and a binding duration fail on both counts — no field exposes them, and no wording reaches them either, because Video Agent always writes and voices its own narration and paces its own timeline. The first group stays native; the second is what Okou composes.
