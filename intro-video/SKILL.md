...
A requirement takes the native route when HeyGen has some mechanism for it — an API field, or an instruction Video Agent acts on. `No avatar` has the second kind: a verified run omitted `avatar_id` entirely and rendered with no digital human in any frame, and a real environment behind the presenter (`presenter.scene: integrated`) is asked for the same way. Narration removal, source-audio retention, page/frame retention and a binding duration have neither kind — no field exposes them, and no wording reaches them, because Video Agent always writes and voices its own narration and paces its own timeline. That is the whole split: the first group is prompted, the second is composed by Okou.
