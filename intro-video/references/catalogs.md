...
Preserve exact public IDs. A look is one appearance of an avatar — one outfit, one preview image, its own `avatar_id` and its own default voice; a group ID names the person and groups that person's looks, so it can never replace the selected look's `avatar_id`. Looks in one group differ in preview dimensions, so crop risk is a property of the look, not of the person. With a selected avatar and `Default` voice, resolve its `defaultVoiceId` and pass that actual voice ID when required. With no presenter on either route, a delegated voice still means an independent public voice matching the brief's language; it does not mean mute, and there is no look to inherit a default from, so resolve one explicitly. A delegated presenter (an older form's `Auto`) is resolved to one concrete public look by the preference order below; record the reason. Submit a native job without `avatar_id` only for `presenter: none`, where omitting it is the point and the prompt states the exclusion. A delegated or unresolved choice must never reach the command as an omitted ID. Letting the agent pick is not the same as resolving a delegated choice, even though the user accepts any look either way: an agent-chosen look is never recorded, so a revision cannot reuse it; its `imageWidth`/`imageHeight` never reach the crop-risk classification that triggers the FRAMING and BACKGROUND notes; and its default voice is unknown until the render exists. Resolving it first is what makes those three work.
...
Apply compatibility checks only to the chosen route. Standalone managed TTS uses a Starfish-compatible voice, while Video Agent has different restrictions. An avatar's default voice may therefore work natively while being unavailable to standalone TTS. If a voice is unavailable, select a compatible public alternative only when voice selection was delegated or the user already authorized a fallback. Otherwise preserve the exact/default selection, report the incompatibility, and let the user choose an alternative. Record any authorized substitution. A fallback choice does not authorize another paid generation job; follow the existing job-recovery and retry rules.
